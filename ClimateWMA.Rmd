...
Shapiro test: if p<0.01, data is not normally distributed.
```{r}
shapiro.test(LC_NDVI$MIN) 
qqnorm(LC_NDVI$MIN); qqline(LC_NDVI$MIN)

shapiro.test(LC_Precip$MIN) 
qqnorm(LC_Precip$MIN); qqline(LC_Precip$MIN)

shapiro.test(LC_Soil$MIN) 
qqnorm(LC_Soil$MIN); qqline(LC_Soil$MIN)

shapiro.test(LC_LST$MIN) 
qqnorm(LC_LST$MIN); qqline(LC_LST$MIN)
```
Test for normality
Shapiro test: if p<0.01, data is not normally distributed.
```{r}
