...
```
Test and plots for differences in WMA
```{r}


kruskal.test(MEDIAN ~ NAME, data = WMA_NDVI) 
epsilonSquared(x = WMA_NDVI$MEDIAN, g = WMA_NDVI$NAME)
kruskal.test(RANGE ~ NAME, data = WMA_NDVI) 
kruskal.test(MAX ~ NAME, data = WMA_NDVI) 

ggplot(WMA_NDVI,aes(x=NAME,y=RANGE))+
  geom_boxplot() + ggtitle("NDVI (0-200) range in communal lands")+ 
...
Tests and plots for soil distribution by communal land
```{r}
