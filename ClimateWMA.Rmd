...
```{r}
shapiro.test(AvgNDVI$AvgNDVI_MEDIAN) 
qqnorm(AvgNDVI$AvgNDVI_MEDIAN); qqline(AvgNDVI$AvgNDVI_MEDIAN)
```
...
LC_Soil<-LC_Soil1040 <- read_csv("LC_Soil1040.csv")
LC_Soil$Soil_Year<-factor(LC_Soil$Soil_Year)
LC_Soil$Soil_Month<-factor(LC_Soil$Soil_Month)
LC_AvgNDVI <- read_csv("LC_AvgNDVI.csv")
LC_AvgNDVI$NDVI_Year<-factor(LC_AvgNDVI$NDVI_Year)
LC_AvgNDVI$NDVI_Month<-factor(LC_AvgNDVI$NDVI_Month)
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
ggplot(LC_AvgNDVI,aes(x=NDVI_Month,y=NDVI_MEDIAN))+
  geom_boxplot() + ggtitle("Median NDVI in different land cover types")+ 
  xlab("Month") + ylab("NDVI") +
  facet_wrap(~NDVI_Year)

LC_AvgNDVI_Plot<-ggplot(LC_AvgNDVI,aes(x=NDVI_Month,y=NDVI_MEDIAN, color=LC))+
  geom_point() + ggtitle("Median NDVI by Land Cover")+ 
  xlab("Month") + ylab("NDVI (0-200)") +
  facet_wrap(~NDVI_Year)
LC_AvgNDVI_Plot1<-LC_AvgNDVI_Plot + scale_color_discrete(name="LC")
LC_AvgNDVI_Plot1
```
```{r}
ggplot(LC_Soil,aes(x=Soil_Month,y=Soil_MEDIAN))+
  geom_boxplot() + ggtitle("Median Soil Moisture in different land cover types")+ 
  xlab("Month") + ylab("Soil Moisture") +
  facet_wrap(~Soil_Year)

LC_Soil_Plot<-ggplot(LC_Soil,aes(x=Soil_Month,y=Soil_MEDIAN, color=LC))+
...
Shapiro test: if p<0.01, data is not normally distributed.
```{r}
shapiro.test(LC_AvgNDVI$NDVI_MEDIAN) 
qqnorm(LC_AvgNDVI$NDVI_MEDIAN); qqline(LC_AvgNDVI$NDVI_MEDIAN)

shapiro.test(LC_Precip$Precip_MEDIAN) 
qqnorm(LC_Precip$Precip_MEDIAN); qqline(LC_Precip$Precip_MEDIAN)

shapiro.test(LC_Soil$Soil_MEDIAN) 
qqnorm(LC_Soil$Soil_MEDIAN); qqline(LC_Soil$Soil_MEDIAN)

shapiro.test(LC_LST$LST_MEDIAN) 
qqnorm(LC_LST$LST_MEDIAN); qqline(LC_LST$LST_MEDIAN)
```
